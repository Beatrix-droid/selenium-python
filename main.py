...
assert page_has_loaded == True, "Error initial login page not loaded correctly" and  browser.save_screenshot("login_page.png")


# click 'login with email and password':
normal_login_button = browser.find_element(By.XPATH, "//button[contains(text(),'Log in with Email and Password')]").click()
...
assert submit_button and password and email, "Error login form not loaded correctly" and browser.save_screenshot("no_submit_btn.png")
...
assert my_name, " homepage not loaded correctly" and  browser.save_screenshot("home_page_not_found.png")
...
assert "Timesheet" in h3_text, "timesheet page not found" and browser.save_screenshot("timesheet_not_found.png")
...
assert day_total,"form for filling in hours not loaded correctly" and  browser.save_full_page_screenshot("hours+worked_form_not_found")
...
if hours_worked == "Day Total: 0h 00m":
    ...
    drop_down.click()
    select_automation=browser.find_element(By.CSS_SELECTOR, ".fab-MenuOption__row").click()
    save_button.click()
    sleep(3)
